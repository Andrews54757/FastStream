...
class Resampler {
  constructor(options) {
    ...
    if (options.nativeSampleRate < this.options.targetSampleRate) {
      console.error('nativeSampleRate is too low. Should have targetSampleRate <= nativeSampleRate');
    }
    this.inputBuffer = [];
  }
}
  ...
  constructor(options) {
    ...
    this.init = async () => {
      this.resampler = new Resampler({
        ...
        targetFrameSize: Math.round(16000 * 2),
      });
...
registerProcessor('tts-helper-worklet', TTSProcessor);
