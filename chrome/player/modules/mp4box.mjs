...
ISOFile.prototype.init = function(_options) {
  ...
  const ftyp = this.add('ftyp').set('major_brand', (options.brands && options.brands[0]) || 'iso4')
      .set('minor_version', 0)
...
ISOFile.prototype.createSingleSampleMoof = function(sample) {
  ...
  if (sample.is_sync) {
    sample_flags = (1 << 25);
  } // sample_depends_on_none (I picture)
  else {
    sample_flags = (1 << 16);
  } // non-sync
...
ISOFile.prototype.resetTables = function() {
  let i;
  let trak; let stco; let stsc; let stsz; let stts; let ctts; let stss;
...
ISOFile.prototype.buildTrakSampleLists = function(trak) {
  let j; let k;
  let stco; let stsc; let stsz; let stts; let ctts; let stss; let stsd; let subs; let sbgps; let sgpds; let stdp;
  let chunk_run_index; let chunk_index; let last_chunk_in_run; let offset_in_chunk; let last_sample_in_chunk;
  let last_sample_in_stts_run; let stts_run_index; let last_sample_in_ctts_run; let ctts_run_index; let last_stss_index; let last_subs_index; let subs_entry_index; let last_subs_sample_index;
...
ISOFile.prototype.updateSampleLists = function() {
  let i; let j; let k;
  let default_sample_description_index; let default_sample_duration; let default_sample_size; let default_sample_flags;
  let last_run_position;
  let box; let moof; let traf; let trak; let trex;
  let sample;
  let sample_flags;
