...
export const MP4BoxStream = function (arrayBuffer) {
...
* Constructor
*/
export const MultiBufferStream = function (buffer) {
...
* License: BSD-3-Clause (see LICENSE file)
*/
export const MPEG4DescriptorParser = function () {
...
BoxParser.createFullBoxCtor("dfLa", function (stream) {
    ...
    do {
        ...
        const typeName = knownBlockTypes[type];
        ...
        if (typeName === "STREAMINFO") { // StreamInfo
            const minBlockSize = stream.readUint16();
            const maxBlockSize = stream.readUint16();
            const minFrameSize = stream.readUint24();
            const maxFrameSize = stream.readUint24();

            // 20 bits, sample rate in Hz
            // 3 bits, number of channels
            // 5 bits, bits per sample
            // 36 bits, total samples in stream
            const half1 = stream.readUint32();
            const half2 = stream.readUint32();

            const sampleRate = (half1 >>> 12) & 0xFFFFF;
            const channels = (half1 >>> 9) & 0x7;
            const bitsPerSample = (half1 >>> 4) & 0x1F;
            const totalSamples = ((half1 & 0xF) << 32) | half2;
            // 128 bits, MD5 signature
            const md5Signature = stream.readUint8Array(16);

            this.samplerate = sampleRate;
            this.samplesize = bitsPerSample + 1;
            this.total_samples = totalSamples;
            this.channel_count = channels + 1;
...
* License: BSD-3-Clause (see LICENSE file)
*/
export const VTTin4Parser = function () {
}
...
export const Textin4Parser = function () {
}
...
* License: BSD-3-Clause (see LICENSE file)
*/
export const ISOFile = function (stream) {
