...
export class VideoAnalyzer extends EventEmitter {
    ...
    async update() {
        ...
        let duration = this.client.duration;



        if (this.outroStatus !== AnalyzerStatus.RUNNING && this.introStatus == AnalyzerStatus.IDLE) {
            let introEnd = Math.min(this.options.introCutoff, duration);
            if (this.shouldLoadPlayer(0, introEnd)) {
                ...
                let reserved = this.referenceFragments(0, introEnd);
        ...
        let outroStart = Math.max(duration - this.options.outroCutoff, this.options.introCutoff);
        if (this.introStatus !== AnalyzerStatus.RUNNING && this.outroStatus == AnalyzerStatus.IDLE && duration - outroStart > 30) {
            if (this.shouldLoadPlayer(outroStart, duration)) {
                ...
                let reserved = this.referenceFragments(outroStart, duration);
    ...
    runAnalyzerInBackground(player, aligner, timeStart, timeEnd, onDone) {
        ...
        let destroyed = false;
        let completed = false;
        let context = player.createContext();
        ...
        const onAnimFrame = () => {
            ...
            aligner.pushVideoFrame(player.getCurrentVideo());
