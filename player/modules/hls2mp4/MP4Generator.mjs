...
export class MP4 {
    ...
    static stts(track) {
        ...
        for (let i = 0; i < track.samples.length; i++) {
            ...
            let delta = sample.duration;
            let last = sample_deltas.length - 1;
        ...
        let len = sample_counts.length;
        let table = new ArrayBuffer(len * 8 + 4 + 4);
        let view = new DataView(table);
        view.setUint32(0, 0);
    ...
    static stsc(track) {
        ...
        for (let i = 0; i < track.chunks.length; i++) {
            let chunk = track.chunks[i];
            let last = first_chunks.length - 1;
        ...
        let len = first_chunks.length;
        let table = new ArrayBuffer(len * 12 + 4 + 4);
        let view = new DataView(table);
        view.setUint32(0, 0);
        view.setUint32(4, len);
        let index = 8;
    ...
    static stsz(track) {
        ...
        let table = new ArrayBuffer(len * 4 + 4 + 4 + 4);
        let view = new DataView(table);
        view.setUint32(0, 0);
        view.setUint32(4, 0);
    ...
    static stco(track) {
        ...
        let len = track.chunks.length;
        let table = new ArrayBuffer(len * 4 + 4 + 4);
        let view = new DataView(table);
        view.setUint32(0, 0);
        view.setUint32(4, len);
        let index = 8;
        for (let i = 0; i < len; i++) {
            let offset = track.chunks[i].offset || 0;
    ...
    static ctts(track) {
        ...
        for (let i = 0; i < track.samples.length; i++) {
            let sample = track.samples[i];
            let offset = sample.cts;
            let last = composition_offsets.length - 1;
    ...
    static stbl(track) {
        if (track.type === 'video') {
            let co = track.use64Offsets ? MP4.co64(track) : MP4.stco(track);
    ...
    static elst(segment_durations, media_times) {
        ...
        let entries = new ArrayBuffer(8 + segment_durations.length * 12);
        let view = new DataView(entries);
    ...
    static edts(track) {
        ...
        let len = track.elst.length;
        for (let i = 0; i < len; i++) {
            let entry = track.elst[i];
